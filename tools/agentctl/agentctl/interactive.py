"""
Interactive Mode for AgentCtl with LLM Chat Support.

Module: agentctl/interactive.py
...
class InteractiveMode:
    """Interactive mode handler for AgentCtl with LLM chat support."""
    ...
    def _init_llm_client(self) -> None:
        ...
        try:
            ...
            from .llm_client import AgentCtlLLMClient
            from .tool_executor import ToolExecutor

            # Let AgentCtlLLMClient handle API key resolution
            # It will load from .env -> llm.yaml -> raise error
            self.llm_client = AgentCtlLLMClient()
    ...
    def _handle_chat(self, user_input: str) -> None:
        ...
        from agentctl.tool_executor import clear_source_tracker
        clear_source_tracker()

        # Check if detailed iteration display is enabled
        show_iteration_details = os.getenv("AGENTCTL_SHOW_ITERATION_DETAILS", "true").lower() == "true"
        verbose_mode = os.getenv("AGENTCTL_VERBOSE_MODE", "true").lower() == "true"
        max_iterations = int(os.getenv("AGENTCTL_MAX_ITERATIONS", "5"))

        # Check if animations are enabled
        animations_enabled = os.getenv("AGENTCTL_ANIMATIONS", "true").lower() == "true"
        ...
        try:
            ...
            from agentctl.tool_executor import get_source_tracker
            from agentctl.iteration_display import (
    ...
    def show_welcome(self) -> None:
        ...
        if animations_enabled:
            ...
            welcome_text = f"""
[bold cyan]AgentCtl v1.0 - Agentic Framework CLI[/bold cyan]
...
Type [bold green]/help[/bold green] for commands, or describe your task in natural language.
    ...
    def cmd_toggle_verbose(self, args: str = "") -> None:
        ...
        current = os.getenv("AGENTCTL_VERBOSE_MODE", "true").lower() == "true"
        new_value = "false" if current else "true"

        # Update environment variable for current session
        os.environ["AGENTCTL_VERBOSE_MODE"] = new_value
        ...
        console.print(f"[dim]To make permanent, edit .env: AGENTCTL_VERBOSE_MODE={new_value}[/dim]")
