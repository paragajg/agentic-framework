"""
LLM Client for AgentCtl Chat Interface.

Module: agentctl/llm_client.py

Provides OpenAI-backed conversational interface for natural language
interaction with AgentCtl features.
"""
...
# Define tools for AgentCtl commands
AGENTCTL_TOOLS = [
    {
        "type": "function",
        "function": {
    ...
    },
    {
        ...
        "function": {
            "name": "show_help",
            "description": "Show help information about AgentCtl commands and capabilities.",
            "parameters": {
                "type": "object",
                "properties": {
                    "topic": {
...
SYSTEM_PROMPT = """You are AgentCtl, a CLI assistant for the Enterprise Agentic Framework.
...
1. Check current state first:
   - LLM config? -> Read .agentctl/llm.yaml
   - MCP servers? -> /mcp list
   - Project config? -> Read .agentctl/config.yaml
...
- If approaching limit (check AGENTCTL_MAX_ITERATIONS env var):
...
class AgentCtlLLMClient:
    ...
    def __init__(
        ...
        Loading Priority:
            ...
            3. Config file (.agentctl/llm.yaml)
            4. Hardcoded defaults

        Args:
            api_key: OpenAI API key (defaults to OPENAI_API_KEY env var)
            model: Model to use (defaults to OPENAI_MODEL env var, then llm.yaml, then gpt-4o-mini)
            max_iterations: Max tool iterations (defaults to AGENTCTL_MAX_ITERATIONS or 5)
        ...
        default_max_iter = int(os.getenv("AGENTCTL_MAX_ITERATIONS", "5"))
        ...
        if not self.api_key:
            raise ValueError(
                ...
                "configure in .agentctl/llm.yaml, or pass api_key parameter."
            )
    ...
    def _load_hyperparameters(self) -> Dict[str, Any]:
        ...
        1. Environment variables (AGENTCTL_MAX_TOKENS)
        2. Config file (.agentctl/llm.yaml hyperparameters)
        3. Defaults
        ...
        env_max_tokens = os.getenv("AGENTCTL_MAX_TOKENS")
        if env_max_tokens:
            try:
                hyperparams["max_tokens"] = int(env_max_tokens)
            except ValueError:
                print(f"Warning: Invalid AGENTCTL_MAX_TOKENS value: {env_max_tokens}")
    ...
    def _build_mcp_server_table(self) -> str:
        ...
        """
        try:
            from agentctl.mcp_gateway_client import MCPGatewayClient
...
**Fallback:** Use /mcp list to see available servers."""
    ...
    def _build_output_mode_instructions(self) -> str:
        ...
        """
        from agentctl.iteration_display import get_output_mode, OutputMode
...
Tool execution details are shown separately in the UI - do not narrate them in your response."""
    ...
    def _load_system_prompt(self, custom_prompt: Optional[str] = None) -> str:
        ...
        if base_prompt is None:
            prompt_path = os.getenv("AGENTCTL_CUSTOM_SYSTEM_PROMPT_PATH")
    ...
    def _get_chat_params(
        ...
        if include_tools:
            params["tools"] = AGENTCTL_TOOLS + CODE_EXECUTION_TOOLS
                ...
                while iteration < max_tool_iterations:
                    ...
                    if not valid_tool_calls or not tool_executor:
                        ...
                        if finish_reason == "length" and collected_content:
                            ...
                            max_continuations = int(os.getenv("AGENTCTL_MAX_CONTINUATIONS", "5"))
