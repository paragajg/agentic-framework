# AgentCtl - Enterprise Agentic Framework CLI
...
```bash
# Initialize project
agentctl init --name my-agent --provider anthropic

# Create agent
agentctl agent new research-agent \
  --role research \
  --capabilities web_search,document_read \
  --output-type research_snippet

# Create skill
agentctl skill new extract-entities \
  --description "Extract named entities from text"

# Configure LLM
agentctl llm config --provider anthropic \
...
agentctl init
# Or with options:
agentctl init --name my-agent --provider anthropic --mcp
```
...
```bash
agentctl agent new <name> [--role ROLE] [--capabilities CAPS]
```
...
```bash
agentctl skill new <name> [--description DESC]
```
...
```bash
agentctl llm config [--provider PROVIDER] [--model MODEL]
```
...
```bash
agentctl mcp add <server> [--scopes SCOPES] [--rate-limit N]
```

**Example:**
```bash
agentctl mcp add github \
...
# Specific service
agentctl logs orchestrator
agentctl logs memory
agentctl logs mcp
```

## Configuration

### Project Configuration (`.agentctl/config.yaml`)
...
### LLM Configuration (`.agentctl/llm.yaml`)
...
# Format code
black --line-length 100 agentctl/
...
AgentCtl integrates with the Enterprise Agentic Framework services:
