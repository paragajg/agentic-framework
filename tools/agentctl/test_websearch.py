...
# Add agentctl to path
sys.path.insert(0, str(Path(__file__).parent))

def test_tool_executor_websearch():
    """Test that tool executor has web search methods."""
    from agentctl.tool_executor import ToolExecutor

    executor = ToolExecutor(config_dir=".agentctl")
...
def test_llm_client_tools():
    ...
    from agentctl.llm_client import AGENTCTL_TOOLS, WEB_SEARCH_TOOLS
    ...
    # Test 2: Check web_search tool is in AGENTCTL_TOOLS
    tool_names = [t['function']['name'] for t in AGENTCTL_TOOLS]
    assert 'web_search' in tool_names, "web_search tool not found in AGENTCTL_TOOLS"

    print(f"✓ web_search tool is registered")
    print(f"  Total tools available: {len(AGENTCTL_TOOLS)}")

    # Test 3: Verify web_search tool definition
    web_search_tool = next((t for t in AGENTCTL_TOOLS if t['function']['name'] == 'web_search'), None)
