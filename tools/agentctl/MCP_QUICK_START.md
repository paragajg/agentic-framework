...
**TLDR:** Just edit YAML files and restart `agentctl`. Everything syncs automatically! ✨
...
├── .agentctl/mcp_servers/     ← Your custom servers (gitignored)
└── mcp_servers/               ← Project-specific servers
```

**Recommendation:** Put your custom MCP servers in `.agentctl/mcp_servers/`
...
```bash
# CLI
agentctl mcp enable <tool_id>
agentctl mcp disable <tool_id>
...
```bash
# Enable verbose mode
export AGENTCTL_VERBOSE_MODE=true
...
```bash
# 1. Create YAML file
cat > .agentctl/mcp_servers/my_server.yaml <<EOF
...
```bash
# 1. Edit YAML file
vim .agentctl/mcp_servers/my_server.yaml
...
No more manual imports. Just edit YAML and restart agentctl. 🎉
