...
agentctl mcp list | grep -A 1 "$TOOL_ID" || echo "  (not registered)"
echo ""
...
agentctl mcp unregister "$TOOL_ID" 2>/dev/null || echo "  (server not registered, continuing...)"

# Re-import
echo ""
echo "Step 2: Importing from $YAML_FILE..."
agentctl mcp import "$YAML_FILE" --format yaml

# Verify
echo ""
echo "Step 3: Verifying update..."
agentctl mcp list | grep -A 1 "$TOOL_ID"
